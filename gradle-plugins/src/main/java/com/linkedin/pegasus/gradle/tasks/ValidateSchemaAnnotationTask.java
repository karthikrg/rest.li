...
public class ValidateSchemaAnnotationTask extends DefaultTask
{
  ...
  public void validateSchemaAnnotation() throws IOException
  {
    ...
    List<URL> handlerJarPathsUrls = new ArrayList<>();

    for (File f : _handlerJarPath)
    {
      handlerJarPathsUrls.add(f.toURI().toURL());
    }

    _classLoader = new URLClassLoader(handlerJarPathsUrls.toArray(new URL[handlerJarPathsUrls.size()]),
    ...
    if (handlerJarPathsUrls.size() == 0 || foundClassNames.size() == 0)
    {
      ...
      throws IOException
  {
        ...
        )
    {
      ...
      String packagePrefix = "com" + FILE_SEPARATOR + "linkedin";
      for (JarEntry e = jarIn.getNextJarEntry(); e != null; e = jarIn.getNextJarEntry())
      {
        if (!e.isDirectory() && e.getName().startsWith(packagePrefix) && !scannedClass.contains(e.getName()))
        {
          scannedClass.add(e.getName());
          checkHandlerClass(toNativePath(e.getName()), foundClasses, packagePrefix);
        }
        jarIn.closeEntry();
      }
  ...
  private boolean checkHandlerClass(String name, List<String> foundClasses, String packagePrefix) throws IOException
  {
    boolean found = false;
    if (name.endsWith(CLASS_SUFFIX) && name.startsWith(packagePrefix))
    {
      int start = name.lastIndexOf(packagePrefix);
