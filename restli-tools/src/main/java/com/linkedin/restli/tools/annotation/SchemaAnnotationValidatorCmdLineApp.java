...
public class SchemaAnnotationValidatorCmdLineApp
{
  ...
  public static void main(String[] args) throws IOException
  {
    ...
    for (DataSchema dataSchema: namedDataSchema)
    {
      ...
      if (result.hasError())
      {
        _log.error("Annotation processing for named data schema {} failed, detailed error: \n",
                   ((NamedDataSchema) dataSchema).getFullName());
        _log.error(result.getErrorMsgs());
        throw new IllegalStateException("ValidateSchemaAnnotation task failed");
      }
      _log.info("Successfully resolved and validated named data schema {}", ((NamedDataSchema) dataSchema).getFullName());
    }
  }
